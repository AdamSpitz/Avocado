...
var Events = MooTools.Events;


var Moousture = window.Moousture = 
new Class(
{
}
);
	...
	*/
    initialize: function(target, options)
    {
		...
		*/
		...
		{
			this.pos.x = evt.pageX; 
			this.pos.y = evt.pageY;
        ...
        // Compatibility patch
        // aaa - let's just hack this for now; not sure what the right long-term mechanism should be. -- Adam\
        //$(target).addEvent('mousemove', _track.bind(this) );
        target.gestureHandler = _track.bind(this);
    },
...
new Class(
{
	...
	*/
    ...
    {
        this.prev = {x:0, y:0};
        this.delay = MooTools.$pick( delay, 20 );
        this.thresh = MooTools.$pick( tHold, 1 );
	...
	*/
    ...
    _monitor: function() {
        var pos = this.prober.probe(); // "var" added by Adam -- Sept. 2010
        
	var deltaX = Math.abs(pos.x - this.prev.x);
	var deltaY = Math.abs(pos.y - this.prev.y);
        if ( deltaX < this.thresh && deltaY < this.thresh )
        {
            if( !this.wasStable )
...
new Class(
{
	...
	*/
	levenDistance: function(v1, v2){
        ...
        return MooTools.$pick(d[v1.length-1][v2.length-1], 0);
    }
}
);
...
new Class(
{
	...
	onMatch: function (mov){
		...
		if( cbLen < 1 || !MooTools.$defined(mov[0]))
			return ;
